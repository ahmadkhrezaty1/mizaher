...
class Admin extends Home
{
    ...
    {           
        ...
        $display_columns = array("#","CHECKBOX",'user_id','avatar','name', 'email', 'manager_type','package_name', 'status','expired_date', 'actions', 'add_date','last_login_at','last_login_ip');
        ...
        {
            if($info[$i]["user_type"] != 'Member' or $info[$i]["manager_type"] == ''){
                $info[$i]["manager_type"] = $info[$i]["user_type"];
            }
            $info[$i]["manager_type"] = $info[$i]["manager_type"]; 
    ...
    public function manager_logs_data()
    {           
        $this->ajax_check();
        $search_value = $_POST['search']['value'];
        ...
        $table="manager_logs";
        $select= array("manager_logs.*","manager_logs.id as id");
        $info=$this->basic->get_data($table,$where,$select,array(),$limit,$start,$order_by,$group_by='');
        $total_rows_array=$this->basic->count_row($table,$where,$count=$table.".id",array(),$group_by='');
        $total_result=$total_rows_array[0]['total_rows'];
