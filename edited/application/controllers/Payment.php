...
class Payment extends Home
{
    ...
    public function fastspring_pay($order, $package_id){
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://api.fastspring.com/orders/'.$order); 
        ...
        if(curl_getinfo($ch, CURLINFO_HTTP_CODE) !== 200) {
            ...
            $days = get_package_expired_date($package_id);
            ...
            $this->session->unset_userdata('current_package_id');
            $this->session->set_userdata('expiry_date',$cycle_expired_date);
            
            $package_info = $this->basic->get_data("package", $where=array("where"=>array("id"=>$package_id)));
    ...
    public function buy_premium_package()
    {
       if($this->session->userdata('license_type') == 'double' && $this->session->userdata('user_type') == 'Member')
       {
           ...
           $payment_method = $this->basic->get_data('transaction_history', array('where' => array('user_id' => $this->user_id), array('payment_type'), '', '', '', 'payment_date,dsc'));
           $data['payment_method'] = isset($payment_method[0]['payment_type']) ? $payment_method[0]['payment_type'] : 'Paypal';
           $data["payment_package"]=$this->basic->get_data("package",$where=array("where"=>array("is_default"=>"0","price > "=>0,"validity >"=>0,"visible"=>"1", "premium_price > "=>0)),$select='',$join='',$limit='',$start=NULL,$order_by='CAST(`price` AS SIGNED)');
    ...
    public function add_package_action() 
    {
        ...
        if($_POST)
        {
            ...
            else
            {
                ...
                $premium_users=$this->input->post('premium_users');
                $premium_fastspring=$this->input->post('premium_fastspring');
                ...
                (
                    ...
                    'premium_fastspring'=>$premium_fastspring,
