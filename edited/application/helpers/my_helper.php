...
if ( ! function_exists('get_user_date')) { 
...
if ( ! function_exists('get_package_expired_date')) { 

  function get_package_expired_date($id) {
    $CI = &get_instance();
    $CI->db->where(['id'=> $id]);
    $user = $CI->db->get('package')->row();
    if(is_object($user)){
...
if ( ! function_exists('uploadImage')) { 
  ...
  function uploadImage($field, $id){
    $ci = &get_instance();
    deleteDirectory("upload/package/$id");
    mkdir('upload/package/'.$id, 0777, true);
    $config['upload_path'] = 'upload/package/'.$id.'/';
...
if ( ! function_exists('uploadImage_p')) { 

  function uploadImage_p($field, $id){
    $ci = &get_instance();
    deleteDirectory("upload/package/p_$id");
    mkdir('upload/package/p_'.$id, 0777, true);
    $config['upload_path'] = 'upload/package/p_'.$id.'/';
    //png, jpg, jpeg, gif, txt, pdf, xls, xlsx, doc, docx
    $config['allowed_types'] = 'gif|jpg|png|jpeg|pdf';
    $ci->load->library('upload', $config);
    $ci->upload->initialize($config);
...
if ( ! function_exists('user_updated_package')) { 

  function user_updated_package($id) {
    $CI = &get_instance();
    $CI->db->where(['id'=> $id]);
    $user = $CI->db->get('users')->row();
    if(is_object($user)){
