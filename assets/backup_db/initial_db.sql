...
INSERT INTO `modules` (`id`, `module_name`, `add_ons_id`, `extra_text`, `limit_enabled`, `bulk_limit_enabled`, `deleted`) VALUES ('109', 'Social Poster - Account Import :  WordPress (Self hosted) ', '0', '', '1', '0', '0');
INSERT INTO `modules` (`id`, `module_name`, `add_ons_id`, `extra_text`, `limit_enabled`, `bulk_limit_enabled`, `deleted`) VALUES ('268', 'Messenger E-commerce', '0', '', '1', '0', '0');
...
ALTER TABLE `messenger_bot_subscriber` ADD INDEX `idx_subscriber_id` (`subscribe_id`);
ALTER TABLE `transaction_history_manual` ADD COLUMN `transaction_id` VARCHAR(150) NOT NULL AFTER `package_id`;
ALTER TABLE `transaction_history_manual` CHANGE `status` `status` enum('0', '1', '2') NOT NULL DEFAULT '0';
ALTER TABLE `payment_config` CONVERT TO CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
ALTER TABLE `email_sending_campaign` ADD `number_of_unique_clickers` INT(11) NOT NULL AFTER `number_of_total_click`, ADD `last_opened_at` DATETIME NOT NULL AFTER `number_of_unique_clickers`, ADD `last_clicked_at` DATETIME NOT NULL AFTER `last_opened_at`, ADD `total_unsubscribed` INT(11) NOT NULL AFTER `last_clicked_at`, ADD `last_unsubscribed_at` DATETIME NOT NULL AFTER `total_unsubscribed`;

ALTER TABLE `email_sending_campaign_send` ADD `is_clicked` ENUM('0','1') NOT NULL DEFAULT '0' AFTER `processed`, ADD `is_unsubscribed` ENUM('0','1') NOT NULL DEFAULT '0' AFTER `is_clicked`, ADD `email_opened_at` DATETIME NOT NULL AFTER `is_unsubscribed`, ADD `unsubscribed_at` DATETIME NOT NULL AFTER `email_opened_at`, ADD `last_clicked_at` DATETIME NOT NULL AFTER `unsubscribed_at`;
...
ALTER TABLE `comboposter_campaigns` ADD `is_child` ENUM('0','1') NOT NULL DEFAULT '0' AFTER `link`, ADD `parent_campaign_id` INT(11) NULL DEFAULT NULL AFTER `is_child`;
ALTER TABLE `comboposter_campaigns` ADD `repeat_times` INT(11) NOT NULL AFTER `schedule_timezone`, ADD `time_interval` INT(11) NOT NULL AFTER `repeat_times`, ADD `full_complete` ENUM('0','1') NOT NULL AFTER `time_interval`;
ALTER TABLE `comboposter_campaigns` CHANGE `parent_campaign_id` `parent_campaign_id` INT(11) NULL DEFAULT NULL;
...
ALTER TABLE `fb_support_category` CHANGE `category_name` `category_name` VARCHAR(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL;
...
CREATE TABLE IF NOT EXISTS `ecommerce_cart` (
  ...
  `payment_method` enum('PayPal','Stripe','Manual','Cash on Delivery') NOT NULL,
...
CREATE TABLE IF NOT EXISTS `ecommerce_config` (
  ...
  `currency` enum('USD','AUD','BRL','CAD','CZK','DKK','EUR','HKD','HUF','ILS','JPY','MYR','MXN','TWD','NZD','NOK','PHP','PLN','GBP','RUB','SGD','SEK','CHF','VND','THB') NOT NULL DEFAULT 'USD',
  `updated_at` datetime NOT NULL,
  PRIMARY KEY (`id`),
